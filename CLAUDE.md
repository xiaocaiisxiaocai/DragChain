...
**认证流程**：前端 `src/utils/auth.ts` 管理 `localStorage` 中的签名 Bearer Token（key: `selection-software-token`），`authVersion` ref 响应式触发 Vue 重新计算。`AuthController` 提供登录和 Token 刷新；每次业务 API 请求由 `RbacMiddleware` 重新从 DB 加载用户状态、角色和权限，确保禁用账号或权限调整实时生效。

**无需 Bearer Token 的端点**：`OPTIONS /api/*`、`GET /api/health`、`POST /login`（兼容别名 `POST /api/auth/login`）、`POST /refresh-token`（兼容别名 `POST /api/auth/refresh-token`）。其中登录仍需有效工号和密码，刷新仍需有效 Refresh Token；除此之外的 `/api/*` 请求均需登录并通过 RBAC 权限匹配。

**生产安全配置**：
- 生产环境必须设置 `DRAGCHAIN_AUTH_SIGNING_KEY`，值为至少 32 字节的高熵随机秘密；通过 IIS 应用程序池环境变量等安全方式注入，不得写入仓库、发布包或日志。
- 新建空 `sensor.db` 首次启动时必须显式设置至少 12 个字符且包含字母、数字和符号的 `DRAGCHAIN_BOOTSTRAP_ADMIN_PASSWORD`；可选设置 `DRAGCHAIN_BOOTSTRAP_ADMIN_EMPLOYEE_NO` 指定初始化管理员工号。系统不提供固定默认密码。
- 升级含历史固定账号的旧库时，安全迁移会在已有非旧版超级管理员可用时禁用旧账号并销毁其原凭据；否则必须显式提供上述强密码完成一次性轮换，缺失时应用拒绝启动。Bootstrap 配置不会覆盖普通现有账号；迁移完成后按组织的秘密管理要求保管或移除进程环境变量。
