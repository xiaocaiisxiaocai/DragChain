  ...
  <PageShell>
    <template #toolbar>
      <div class="toolbar-title">
        <strong>模块库</strong>
        <span>{{ filteredRows.length }} 个模块</span>
      </div>
    ...
    <el-table v-loading="loading" :data="filteredRows" border stripe height="100%" class="admin-table">
      ...
      <el-table-column label="包含内容" min-width="260" show-overflow-tooltip>
        <template #default="{ row }">{{ describeModule(row) }}</template>
      </el-table-column>
...
const form = reactive<CreatePipeModule>({
  name: '',
  description: '',
  items: []
});
...
const filteredRows = computed(() => {
  ...
  return rows.value.filter(row => `${row.name} ${row.description} ${describeModule(row)}`.toLowerCase().includes(q));
});

async function load() {
  loading.value = true;
  try {
    rows.value = await pipeModulesApi.getAll();
  } finally {
    loading.value = false;
  }
}
...
function startEdit(row: PipeModule) {
...
async function save() {
  ...
  if (!payload.items.length) {
    ElMessage.warning('模块至少需要包含一条管线');
    return;
  }

  if (editingId.value) {
    await pipeModulesApi.update(editingId.value, payload);
  } else {
    await pipeModulesApi.create(payload);
  }
...
async function remove(id: number) {
  await ElMessageBox.confirm('确认删除这个模块？', '删除确认', { type: 'warning' });
  await pipeModulesApi.delete(id);
